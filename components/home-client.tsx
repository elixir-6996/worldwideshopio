...
/* ─── Data ───────────────────────────────────────────────────────── */
/** Turns a category label into the slug the products browser expects. */
    ...
    <FadeUp delay={index * 0.08}>
      <div className="group relative flex flex-col">
        ...
        <button
          ...
          className={`mt-3 w-full flex items-center justify-center gap-2 text-xs font-semibold tracking-widest uppercase h-10 rounded-xl transition-all duration-300 border disabled:opacity-60 ${
            added
...
export function HomeClient({
  ...
  // Best sellers are the highest rated items; trending fills in the rest so the
  // two rails never repeat the same product.
  const byRating = [...products].sort((a, b) => b.rating - a.rating)
  const bestsellers = byRating.slice(0, 4)
  const trending = byRating.slice(4, 8).length ? byRating.slice(4, 8) : byRating.slice(0, 4)
      ...
      >
        <div className="mx-auto max-w-[1400px] px-4 md:px-6">
          <div className="flex items-center gap-0 overflow-x-auto scrollbar-hide">
            <Link
              href="/products"
              className="shrink-0 px-4 py-3 text-xs font-medium tracking-wide whitespace-nowrap transition-colors border-b-2 -mb-px border-primary text-primary"
            >
              All Categories
            </Link>
            {categories.map((cat) => (
              <Link
                key={cat}
                href={`/products?cat=${encodeURIComponent(slugify(cat))}`}
                className="shrink-0 px-4 py-3 text-xs font-medium tracking-wide whitespace-nowrap transition-colors border-b-2 -mb-px border-transparent text-muted-foreground hover:text-foreground hover:border-white/20"
              >
                {cat}
              </Link>
            ))}
