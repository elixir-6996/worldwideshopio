...
/**
 * Serialises the cart into the base64url `?cart=` token the checkout route
 * parses. Only slugs and quantities travel in the URL — prices are always
 * re-read from the database on the server.
 */
function encodeCart(cart: CartItem[]) {
  const json = JSON.stringify(toCartPayload(cart))
...
export function CartClient({
  ...
  const baseShipping = baseShippingFor(subtotal, 'standard', rates)

  const applyPromo = async () => {
    const result = await validateCoupon({ code: promoCode, subtotal, shipping: baseShipping })
  ...
  const shipping = baseShipping - (appliedPromo?.shippingSavings ?? 0)
  const total = subtotal - discount + shipping

  return (
    <div className="min-h-screen flex flex-col">
      <Navbar cartCount={cart.reduce((a, i) => a + i.quantity, 0)} />
                ...
                <div className="flex flex-col gap-3 text-sm">
                  ...
                  {shipping > 0 && (
                    <p className="text-xs text-muted-foreground">
                      Add <DualPrice usdCents={(rates.freeShippingThreshold - subtotal) * 100} />{' '}
                ...
                >
                  <Link
                    href={`/checkout?cart=${encodeCart(cart)}${
                      appliedPromo ? `&coupon=${encodeURIComponent(appliedPromo.code)}` : ''
                    }`}
                  >
